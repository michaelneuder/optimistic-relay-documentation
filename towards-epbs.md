...
There are tradeoffs between ePBS and `mev-boost` as highlighted by Barnabé at [Devconnect](https://youtu.be/jQjBNbEv9Mg?t=943) and in [Notes on PBS](https://barnabe.substack.com/i/82304191/market-structure-and-allocation-mechanism). This roadmap explores the 
...
The relay still validates each block it receives, but this validation happens asynchronously, thus
is no longer part of the critical path. This demonstrates the main objective of this roadmap — "to reduce the duties of the relay 
...
Here we remove the additional latency of waiting for the whole block execution body to 
...
> This design has an additional practical benefit. The relay is no longer able to censor because the bid is eligible when the header is parsed. The relay doesn't yet have information about the transactions in the execution body, so it can't censor based on those transactions. 


## Optimistic Relay v3 — "Relay as an oracle"
The figure below demonstrates the critical path under Optimistic Relaying v3.
...
#### What is the critical path?
Note that now the relay is no longer in the critical path! The builders 
...
The figure below demonstrates the critical path under ePBS.
...
block that differs from the winning header, the committee members simply do not attest to it. 
 ...
 > The first committee attests 
to the timely publication of a signed header and the remaining committees attest to the timely publication of a signed block, which is exact set of actions that the 
relay listens for in v3.
